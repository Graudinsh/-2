#Этот код я создал для того чтобы люди которые проверяют этот код смогли пройти эту игру что я сам создал.СЮЖЕТ:В начале игры вы залезаете в дом хотя его ограбить,далее в игре.#СОЗДАТЕЛЬ:Граудиньш Илья
end_gema_flag = False
